...
const navItems: Item[] = [
  { name: 'Discovery Process', href: '/', icon: Search },
  { name: 'Client Projects', href: '/projects', icon: FileText },
...
export default function Sidebar() {
  ...
  return (
    <aside className="fixed left-0 top-0 w-64 h-screen border-r bg-white flex flex-col z-10 overflow-hidden transition-[width]">
      {/* Brand row with the Warren logo and product name */}
      <div id="sb-header" className="relative flex items-center gap-2 px-4 py-3 h-14 border-b flex-shrink-0">
        <div className="w-12 h-12 bg-gradient-to-br from-[#64B37A] to-[#2F6D49] rounded-full flex items-center justify-center">
          <span className="text-white font-bold text-lg">W</span>
        </div>
        <span className="text-sm font-semibold text-[#0E2A23]" data-sb-label>Warren Hatch</span>
        <button
          id="sb-toggle"
          ...
          onClick={() => {
            ...
            const logo = document.querySelector('[data-sb-logo]');
            if (logo) {
              (logo as HTMLElement).style.display = isCollapsed ? 'none' : 'block';
            }
      ...
      <div className="border-t p-3 bg-white flex-shrink-0">
        <div className="flex items-center space-x-3 mb-3">
          ...
          <div className="flex-1">
            <div className="font-medium text-gray-900 text-sm">Discovery User</div>
            <div className="text-xs text-gray-500">Warren Client</div>
          </div>
        </div>
